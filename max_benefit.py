...
def min_cost(truck_list,distance):
    ...
    for t in truck_list:
        if(t.alread_ran+distance>t.minmum_run):
            pay=t.rate+(t.alread_ran+distance-t.minmum_run)*(t.rate*120/100)
        else:
            pay=t.rate
...
truck_list.append(Truck("l4",25,3400,1500,[datetime(2020,4,26),datetime(2020,5,25)]))
truck_list.append(Truck("l5",29,2800,2800,[datetime(2020,4,1),datetime(2020,4,30)]))


print(min_cost(truck_list,600))
